...
function seismic_scatter_2D(region, trim_year, magnitude_threshold, with_image, img=nothing)
	...
	# save("romania_mag_$magnitude_threshold.png", fig)
	fig
end
...
ro_counted = join_on_counted_no_mag_bounds(romania,trim_year,0)

# ╔═╡ 3410c384-2928-4503-ac0d-db8b0ddc65cd
begin
	bar(ro_counted.Year, ro_counted.count, 
...
begin
	mags=[]
	for minmag=0:7
		push!(mags,ro_counted[!,Symbol("count_mag_$minmag")])
	end

	ticklabels = string.(collect(minimum(ro_counted.Year):maximum(ro_counted.Year)))
...
begin
	...
	events=0
	for minmag=0:7
		events+= sum(ro_counted[!,Symbol("count_mag_$minmag")])
	end
	events == sum(ro_counted[!,:count])
end

# ╔═╡ 3a122a51-8d5a-4d3f-aec1-8647f2d12a7b
for minmag=0:7
	maxmag=minmag+1
	ro_counted_mag_extra = join_on_counted(romania,trim_year,minmag,maxmag)

	leftjoin!(ro_counted, ro_counted_mag_extra, on=:Year,makeunique=true)
	ro_counted[!,Symbol("count_mag_$minmag")] = replace(ro_counted[!,Symbol("count_mag_$minmag")], missing => 0)
end
...
ro = load("./maps/romania_wikipedia.png"); nothing

# ╔═╡ 886b94f6-e12f-4d3a-a043-63a0503458e8
seismic_scatter_2D(romania, trim_year, magnitude_threshold, with_map, ro)

# ╔═╡ 31e2cb67-c945-4146-a9d2-296f23562d41
md"#### 3D scatter"

# ╔═╡ 75db2fcd-22cf-4f42-8cfc-e07a70798c2d
seismic_scatter_3D(romania, trim_year, magnitude_threshold, with_map, ro)

# ╔═╡ 8a94eb20-18e0-429b-a42e-d203328d14ce


# ╔═╡ c298c1ef-7ddb-4af1-b993-0d7a3f3fa9d4
md"# Vrancea seismic region"

# ╔═╡ c56c918e-e307-4439-86c2-585a10151dad
# Setting the boundaries for Vrancea region
# Taken by visualizing the earthquakes in Romania
vrancea = romania[(romania.Longitude .>= 26 ) .& (romania.Longitude .<= 27) .&
				  (romania.Latitude .>= 45.2) .& (romania.Latitude .<= 46) .&# ,:]
				  (romania.Depth .>= 50) .& (romania.Depth .<= 200) ,:]


# ╔═╡ f378abea-6866-473f-ab0f-150c52e09acd
vr = load("./maps/vrancea_wikipedia.png"); nothing

# ╔═╡ 229b3b6b-05c8-4a41-ba05-ff6f8ab238ef
seismic_scatter_2D(vrancea, trim_year, magnitude_threshold, with_map, vr)

# ╔═╡ 3782130c-ce66-4ec4-b564-5a0a1957aefe
seismic_scatter_3D(vrancea, trim_year, magnitude_threshold, with_map, vr)

# ╔═╡ 0a97efe1-44ce-4e63-8567-c80d467ca27b
